...
def WordScrape(userLet):
    ...
    # filter out words that don't contain any of the letters
    # and words that are too short or too long
    for letter in userLet:
        for word in words:
            if letter in word and 3 <= len(word) <= userLen:
...
# (0,2) submit button
btn = Button(fr1, text="Submit", fg="#FF4500",
             font=(fonty, n), command=submitbtn)
btn.pack(side=LEFT)
...
tag = Label(fr2, text="An original project by",
            fg='#737373', bg='#EEE', font=(fonty, 10))
tag.pack(side=LEFT)

btn2 = Button(fr2, text="Sahas Ramesh", fg="#FF4500", bd=0, activebackground='#EEE',
              highlightbackground='#EEE', highlightcolor='#EEE', highlightthickness=0, font=(fonty, 10), command=tagbtn)
btn2.pack(side=LEFT)
